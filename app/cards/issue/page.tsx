...
function IssueCardContent() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!userProfile?.id || !accountId || cardholderName.trim().length < 2) {
      toast.error('Choose an account and enter the cardholder name.')
      return
    }
    if (pin && !/^\d{4}$/.test(pin)) {
    ...
    try {
      const response = await fetch('/api/cards', {
        ...
        body: JSON.stringify({ action: 'issue', userId: userProfile.id, accountId, type, brand: 'visa', design: 'classic', cardholderName: cardholderName.trim(), pin: pin || undefined }),
      })
