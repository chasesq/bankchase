...
function CardDetailContent() {
  ...
  const [replacementReason, setReplacementReason] = useState<'lost' | 'stolen' | 'damaged' | null>(null)
  ...
  const submitReplacement = () => {
    if (!replacementReason) {
      toast.error('Choose whether your card is lost, stolen, or damaged.')
      return
    }
    setIsReplaceDialogOpen(false)
    void updateCard('replace', { reason: replacementReason })
  ...
  {isReplaceDialogOpen ? <div role="presentation" className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 p-4 backdrop-blur-sm" onClick={() => setIsReplaceDialogOpen(false)}><section role="dialog" aria-modal="true" aria-labelledby="replace-card-title" className="w-full max-w-md rounded-2xl border border-border bg-card p-6 shadow-xl" onClick={(event) => event.stopPropagation()}><div className="flex items-start justify-between gap-4"><div><h2 id="replace-card-title" className="text-lg font-semibold text-foreground">Replace card</h2><p className="mt-1 text-sm text-muted-foreground">Choose the reason so we can securely cancel this card and issue a replacement.</p></div><button type="button" aria-label="Close replacement dialog" onClick={() => setIsReplaceDialogOpen(false)} className="text-muted-foreground hover:text-foreground">×</button></div><div className="mt-5 grid gap-2">{(['lost', 'stolen', 'damaged'] as const).map((reason) => <button key={reason} type="button" aria-pressed={replacementReason === reason} onClick={() => setReplacementReason(reason)} className={`rounded-xl border p-3 text-left capitalize transition-colors ${replacementReason === reason ? 'border-primary bg-primary/10 text-foreground' : 'border-border text-muted-foreground hover:bg-muted'}`}>{reason}</button>)}</div><div className="mt-6 flex justify-end gap-3"><button type="button" onClick={() => setIsReplaceDialogOpen(false)} className="rounded-lg border border-border px-4 py-2 text-sm font-medium text-foreground hover:bg-muted">Cancel</button><button type="button" disabled={!replacementReason || isUpdating} onClick={submitReplacement} className="rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground disabled:cursor-not-allowed disabled:opacity-50">{isUpdating ? 'Submitting…' : 'Request replacement'}</button></div></section></div> : null}
  </main>
}
