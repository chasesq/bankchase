    ...
    <main className="flex min-h-svh items-center justify-center bg-background px-5 py-10 text-foreground">
      <section className="w-full max-w-md rounded-xl border border-border bg-card p-6 shadow-sm sm:p-8">
        ...
        <form onSubmit={handleSubmit} className="mt-7 flex flex-col gap-5">
          ...
          <label className="flex flex-col gap-2 text-sm font-medium">{isSignUp ? "Username" : "Admin email or username"}<input className="rounded-md border border-border bg-background px-3 py-3 font-normal outline-none focus:ring-2 focus:ring-ring" value={username} onChange={(e) => setUsername(e.target.value)} required autoComplete="username" /></label>
