...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env["DEFAULT_TEAM_NT7_HY_BASE_URL"].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env["DEFAULT_TEAM_NT7_HY_LOG"] or 'warn' if it isn't set.
   */
...
export class DemoAPIScalarGalaxy {
   ...
   * @param {string | null | undefined} [opts.webhookSecret=process.env["DEFAULT_TEAM_NT7_HY_WEBHOOK_SECRET"] ?? null]
   * @param {Environment} [opts.environment=production] - Specifies the environment URL to use for the API.
   * @param {string} [opts.baseURL=process.env["DEFAULT_TEAM_NT7_HY_BASE_URL"] ?? https://galaxy.scalar.com] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('DEFAULT_TEAM_NT7_HY_BASE_URL'),
    ...
    webhookSecret = readEnv('DEFAULT_TEAM_NT7_HY_WEBHOOK_SECRET') ?? null,
    ...opts
  }: ClientOptions = {}) {
    ...
    if (baseURLOverridden && options.environment)
      throw new Errors.DemoAPIScalarGalaxyError(
        'Ambiguous URL; The `baseURL` option (or DEFAULT_TEAM_NT7_HY_BASE_URL env var) and the `environment` option are given. If you want to use the environment you must pass baseURL: null',
      );
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('DEFAULT_TEAM_NT7_HY_LOG'), 'process.env["DEFAULT_TEAM_NT7_HY_LOG"]', this) ??
      defaultLogLevel;
    ...
    const customHeadersEnv = readEnv('DEFAULT_TEAM_NT7_HY_CUSTOM_HEADERS');
    if (customHeadersEnv) {
